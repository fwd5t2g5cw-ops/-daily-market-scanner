...
def build_features(d):
    ...
    # High-volume days that do NOT produce downside: possible absorption/support.
    absorb=((vol_rel>=1.5) & (ret>=-0.01) & (clv>=0.5)).astype(int)
    # High volume + small price progress = effort without downside result.
    effort_no_down=((vol_rel>=1.5) & (ret.abs()<=0.012) & (clv>=0.45)).astype(int)
    # Heavy down-volume days that close well off lows: downside rejection.
    down_reject=((vol_rel>=1.3) & (ret<0) & (clv>=0.65)).astype(int)
    # Undercut prior 10d low intraday, but reclaim that prior low on close.
    prior_low=l.shift(1).rolling(10).min()
    undercut_reclaim=((l<prior_low) & (c>prior_low)).astype(int)
    # Up-volume/down-volume balance over 20d.
    upv=v.where(ret>0,0).rolling(20).sum(); dnv=v.where(ret<0,0).rolling(20).sum()
    updown=(upv/dnv.replace(0,np.nan)).clip(0,10)
    # Price resilience: downside volatility relative to upside volatility.
    ...
    # Are pullbacks occurring on lighter volume than advances?
    upavg=v.where(ret>0).rolling(20).mean(); dnavg=v.where(ret<0).rolling(20).mean()
    ...
    f['highvol_close_strength10']=clv.where(vol_rel>=1.3).rolling(10,min_periods=2).mean()
    f['volume_cluster10']=(vol_rel>=1.3).astype(int).rolling(10).sum()
    ...
    # outcome uses future open gaps only; no future data enters features.
    gap=(o/c.shift(1)-1)*100
    arr=gap.to_numpy(); y=np.zeros(len(x),dtype=int)
    maxg=np.full(len(x),np.nan)
    for i in range(len(x)):
        z=arr[i+1:min(i+1+FWD,len(x))]
        if len(z):
            maxg[i]=np.nanmax(z)
            y[i]=int(maxg[i]>=POWER)
    f['future_power_gap']=y; f['max_gap_next10']=maxg
    return f

def main():
    src=pd.read_csv(SRC)
    symbols=sorted(src.symbol.astype(str).unique())
    frames=[]
    for n,s in enumerate(symbols,1):
        ...
        f=f[(f.date>=START)&(f.date<=END)].dropna()
        frames.append(f)
    z=pd.concat(frames,ignore_index=True)
    z.to_csv(OUT/'stock_days.csv',index=False)
    y=z.future_power_gap.astype(bool); ng=int(y.sum()); nn=int((~y).sum())
    ...
    rows=[]
    for c in feats:
        a=z.loc[y,c]; b=z.loc[~y,c]
        sd=np.sqrt((a.var()+b.var())/2); smd=(a.mean()-b.mean())/sd if pd.notna(sd) and sd>0 else np.nan
        rows.append({'feature':c,'gap_mean':a.mean(),'normal_mean':b.mean(),'std_diff':smd})
    summ=pd.DataFrame(rows).sort_values('std_diff',key=lambda q:q.abs(),ascending=False)
    summ.to_csv(OUT/'feature_summary.csv',index=False)

    # Forward tests of simple upper/lower quantile tails learned on first half, tested on second half.
    train=z[z.date<'2024-01-01']; test=z[z.date>='2024-01-01'].copy()
    tests=[]
    for c in feats:
    ...
    tails=pd.DataFrame(tests).sort_values('lift',ascending=False)
    tails.to_csv(OUT/'forward_tail_tests.csv',index=False)

    # accumulation score from behavior-only signals; thresholds fixed from 2022-23 training.
    tr=train
    qs={c:tr[c].quantile(.75) for c in ['absorb20','effort_no_down20','down_reject20','undercut_reclaim20','updown_vol20','resilience20','vol_asym20','highvol_close_strength10']}
    score=np.zeros(len(test),dtype=int)
    for c,t in qs.items(): score += (test[c]>=t).to_numpy()
    test['behavior_score']=score
    score_rows=[]; base=test.future_power_gap.mean()
    for k in range(2,9):
        m=test.behavior_score>=k
        hit=test.loc[m,'future_power_gap'].mean() if m.sum() else np.nan
