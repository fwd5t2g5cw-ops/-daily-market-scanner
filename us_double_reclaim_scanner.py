...
BENCHMARK = 'SPY'
UNIVERSE_FILE = Path('data/us_1b_universe.txt')
...
def _batch_daily(symbols: list[str], chunk: int = 180) -> dict[str, pd.DataFrame]:
    out = {}
...
def main():
    ...
    if not UNIVERSE_FILE.exists():
        raise RuntimeError('data/us_1b_universe.txt not found; run Build US $1B+ Universe first')
    symbols=[s.strip().upper() for s in UNIVERSE_FILE.read_text().splitlines() if s.strip()]
    symbols=list(dict.fromkeys(symbols))
    print('US $1B+ universe',len(symbols))
    ...
    for sym in symbols:
        ...
        shortlist.append({'symbol':sym,'breakout_level':level,'breakout_age':age,'rs_prev_pct':rs,
                          'congestion_score':cong,'congestion_status':cong_status,'_daily':d})

    print('Technical shortlist before intraday',len(shortlist))
    ...
    for x in shortlist:
        ...
        rows.append({'symbol':sym,'status':status,'grade':final,'quality_score':quality,
                     ...
                     'ema20_breakout_distance_pct':round(cluster,3),'rs_vs_spy_pct':round(rs,2),
                     'pct_below_52w_high':round(pct_below,2),'breakout_age_days':x['breakout_age'],
                     'undercut_vs_breakout_pct':round(undercut,2),'shallow_bonus':shallow,
                     'headroom_bonus':headroom,'congestion_score':x['congestion_score'],
    ...
    if out.empty:
        out.to_csv(OUTDIR/'us_double_reclaim_all.csv',index=False)
        out.to_csv(OUTDIR/'us_double_reclaim_top30.csv',index=False)
    ...
    out=out.sort_values(['_s','_g','quality_score','rs_vs_spy_pct'],ascending=[True,True,False,False]).drop(columns=['_s','_g'])
    out.to_csv(OUTDIR/'us_double_reclaim_all.csv',index=False)
    out.head(30).to_csv(OUTDIR/'us_double_reclaim_top30.csv',index=False)
    ready=out[out['status']=='READY_NOW']; ready.to_csv(OUTDIR/'us_double_reclaim_ready_now.csv',index=False)
    print('\n=== TOP 30 (US $1B+) ===')
    cols=['symbol','status','grade','quality_score','current_price','breakout_level','ema20_live',
          'ema20_breakout_distance_pct','rs_vs_spy_pct','pct_below_52w_high','congestion_score']
