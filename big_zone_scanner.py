...
def analyze(ticker,market):
    ...
    c,h,l,v=map(_series,[d.Close,d.High,d.Low,d.Volume]); ema20=c.ewm(span=EMA_LEN,adjust=False).mean(); avgvol=v.rolling(VOL_LEN).mean(); prior=h.shift(1).rolling(RESISTANCE_BARS).max(); threshold=prior*(1+MIN_BREAKOUT_PCT/100)
    bo=(c>threshold)&(c.shift(1)<=prior); idx=np.flatnonzero(bo.fillna(False).to_numpy())
    if len(idx)==0:return None
    last=int(idx[-1]); age=len(d)-1-last
    if age<0 or age>MAX_BO_AGE:return None
    level=float(prior.iloc[last]); price=float(c.iloc[-1]); e20=float(ema20.iloc[-1]); low=float(l.iloc[-1]); vol=float(v.iloc[-1]); av=float(avgvol.iloc[-1])
    ema_dist=min(abs(price/e20-1),abs(low/e20-1))*100 if e20>0 else 999; level_dist=min(abs(price/level-1),abs(low/level-1))*100 if level>0 else 999
    bo_score=3 if age<=5 else 2 if age<=15 else 1; ema_score=3 if ema_dist<=2 else 2 if ema_dist<=5 else 1 if ema_dist<=8 else 0; level_score=3 if level_dist<=2 else 2 if level_dist<=5 else 1 if level_dist<=8 else 0
    contraction=bool(pd.notna(av) and vol<av); score=bo_score+ema_score+level_score+(1 if contraction else 0); status='READY' if score>=8 and ema_score>=2 and level_score>=2 else 'WATCH' if score>=6 else 'EARLY'
...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--us-file',default='data/us_universe.txt'); ap.add_argument('--tsx-file',default='data/tsx_universe.txt'); ap.add_argument('--hk-file',default='data/hk_universe.txt'); ap.add_argument('--outdir',default='big_zone_results'); a=ap.parse_args(); out=Path(a.outdir); out.mkdir(parents=True,exist_ok=True)
    rows=[]
